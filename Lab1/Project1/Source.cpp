...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) 
{
	...
	case WM_CREATE: {
		...
		customFont = CreateFont(31, 0, 0, 0, FW_DONTCARE, FALSE, TRUE, FALSE,
	...
	case WM_GETMINMAXINFO: {
		...
		winSize->ptMaxTrackSize.x = 900;
		winSize->ptMaxTrackSize.y = 650;
		break;
	}
	case WM_COMMAND: {
		...
		case IDC_BUTTON2: {
			...
			SetWindowText(textBox1, L"Type here->");
			textEntered = false;
	...
	case WM_CLOSE: {
		...
		MessageBox(hwnd, L"to exit type 'quit' in upper box and press Append ", L"Program termination", MB_OK | MB_ICONINFORMATION);
		break;
	}
	case WM_SYSCOMMAND: {
	...
	case WM_SIZE: {
		...
		MoveWindow(label1, (width - 440) / 3, 10, 440, 20, TRUE);
	...
	case WM_CTLCOLORSTATIC: {
		if ((HWND)lParam == label1) {
			...
			SetBkMode((HDC)wParam, TRANSPARENT);
			SetTextColor((HDC)wParam, textEntered ? RGB(15, 170, 40) : RGB(17, 40, 47));
		}
	...
	case WM_CTLCOLOREDIT: {
		if (IDC_TEXT1 == GetDlgCtrlID((HWND)lParam)) {
			HBRUSH textBoxBrush = CreateSolidBrush(RGB(34, 139, 34));
			SetBkColor((HDC)wParam, RGB(34, 139, 34));
			SetTextColor((HDC)wParam, RGB(81, 72, 79));
			return(INT_PTR)textBoxBrush;
		}
		break;
	}
